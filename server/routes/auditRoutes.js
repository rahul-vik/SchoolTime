...
export function createAuditRoutes(db) {
  ...
  router.get("/audit-logs", (req, res) => {
    ...
    const logs = db.prepare(`SELECT a.id, a.action, a.entity_type, a.entity_id, a.metadata_json, a.created_at, u.full_name
  ...
  router.get("/audit-logs/export.csv", (req, res) => {
    ...
    const { whereSql, args } = buildAuditWhere(req.query, req.auth.orgId);
    const rows = db.prepare(`SELECT a.created_at, IFNULL(u.full_name,'System') actor, a.action, a.entity_type, IFNULL(a.entity_id,'') entity_id, IFNULL(a.metadata_json,'') metadata_json
        FROM audit_logs a LEFT JOIN users u ON u.id = a.user_id
        WHERE ${whereSql}
        ORDER BY a.created_at DESC LIMIT ?`)
      .all(...args, limit);
