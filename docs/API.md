...
- **`POST /timetable/apply-edit`** — requires `canConfigureTimetable`. Body: `{ runId?, entries?, sourceState?, operation: "SWAP"|"MOVE"|"ADD", source?, target, subjectId?, teacherId? }`. **`SWAP`/`MOVE`:** `source` + `target` cells. **`ADD`:** `target` free cell + `subjectId` + `teacherId`. Validates the full operation; on success updates `timetable_runs.entries_json`, `report_json` (manual edit audit plus **live** `unscheduled`, `totalScheduled`, `totalRequired`, and `objective` recomputed from `entries` via `shared/recomputeTimetableReport.js`), run `score`/`status`, and `tenant_state.lastGeneratedTimetable` when the run matches. Returns `{ ok, runId, operation, kind, entries, validation, timetable }` where `timetable.report` reflects current entries; generate-time fields such as `rejections` and `optimization` are preserved. **400** with `{ error, reasons[] }` on validation failure.
...
- **`GET /timetable/latest` response** — `{ run, timetable }` where `timetable` includes `entries`, `report`, `runId`, `generatedAt`, and **`sourceState`** parsed from the latest row’s `state_json` when present (older rows may omit it; clients fall back to live `GET /state` lists). `report.unscheduled`, `totalScheduled`, `totalRequired`, and `score`/`status` are recomputed from stored `entries` + `sourceState` so manual edits are reflected without regenerating.
