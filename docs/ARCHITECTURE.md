...
| Teachers (subjects, mediums, caps, continuity, division allow/exclude) | Yes | `teacherSubjects` narrows eligible teachers when present. **`divisionSubjectTeacherLocks`** pins one teacher per division+subject before greedy placement (also backfilled from singleton `teacherSubjects` rows on migrate). |
...
Located in `server/engine.js` with greedy tuning helpers in `server/legacyEngineImprovements.js`, phased ordering in `shared/enginePlacementPhases.js`, and post-placement optimizers in `server/legacyEngineOptimizers.js`. HTTP generate uses `server/timetableSolverRunner.js` → `runTimetableGenerationEngine` (async) which defaults to the same `runTimetableEngine` implementation. **`POST /timetable/generate`** runs **`runTenantPreflightCheck`** on the scoped tenant first and returns **400** when blocking rule contradictions exist. **`runTenantFeasibilityReport`** does **not** block generate (warnings and errors are attached as **`report.feasibility`** with per division+subject **`rows`**).
...
2. **Pre-seed division+subject teacher locks** from persisted **`divisionSubjectTeacherLocks`** (`[{ divisionId, subjectId, teacherId }]`), then from **`teacherSubjects`** when exactly one teacher applies (avoids the first greedy placement locking the wrong teacher). Multiple `teacherSubjects` rows for the same pair without a team-teaching flag still use single-teacher lock behavior after first placement.
...
5. **Phased main placement** (`shared/enginePlacementPhases.js`): divisions are ordered **higher standard first** (`sortDivisionsHigherStandardFirst` in `shared/schoolDisplayOrder.js`, e.g. Std 12 before Std 8 before Std 1), then **rotated per restart seed** for diversity; within each division, allocate in order **CONSTRAINED** (active `INCLUDE_ONLY`) → **LAB_PRACTICAL** (tries adjacent double blocks when category is `PRACTICAL`) → **CORE** → **LANGUAGE** → **NON_CORE** → **REMAINING**; within each phase, subjects are **hardest-first** (`sortSubjectsHardestFirst`). Lock repair, division-centric fill, local-search shortages, and post-placement optimizers use the same higher-class-first division order. For each needed period, **`findEligibleTeacher`** prefers **division specialists** before generalists when there is no explicit `teacherSubjects` list. If a subject cannot finish after scanning days, **backtrack** by undoing the last N **unpinned** placements and retry. Stats: `report.optimization.subjectOrder: "phased"`, `report.optimization.flowPhases`.
6. **Multi-restart:** run the full greedy attempt multiple times (default **4** / **5** / **3** restarts for **STRICT** / **BEST_FIT** / **OPTIMAL**; override with **`LEGACY_ENGINE_RESTARTS`** or **`legacyEngineOptions.restarts`**). Keep the attempt with the best lexicographic **`report.objective`**, then score, then fewest **unscheduled** shorts.
7. **BEST_FIT** / **OPTIMAL** soft passes (relax day/slot excludes only) when those modes are selected.
8. **Division-centric fill** (after lock repair): per division, fill empty cells with any still-short subject. Stats: `report.optimization.divisionCentricFill`.
9. **Lock repair (default four rounds, configurable):** strip unpinned lessons for short **(division, subject)** pairs, clear locks, retry gap fill; re-apply class-teacher rules. Tune with **`LEGACY_ENGINE_LOCK_REPAIR_ROUNDS`** or **`legacyEngineOptions.lockRepairRounds`**. Stats: `report.lockRepair`.
10. **Local search** (`server/legacyEngineLocalSearch.js`): lexicographic gap-fill, relocations, swaps; iteration budget scales slightly with division count. Tune with **`LEGACY_ENGINE_LOCAL_SEARCH_*`** / **`legacyEngineOptions.localSearch*`**.
11. **Post-placement optimizers** (`server/legacyEngineOptimizers.js`, before Free fill): **gap utilization** (up to three passes), **cross-division unblock** (bounded teacher moves), **distribution**, **period spread**, **teacher load balance**. Subjects may set **`requiresDoublePeriod`** to prefer adjacent double blocks when weekly count allows pairs. Stats: `report.optimization.gapUtilization`, `.crossDivisionUnblock`, `.distribution`, `.periodSpread`, `.teacherLoadBalance`.
12. Fill remaining unassigned lesson slots with `isFreePeriod`.
13. Compute unscheduled requirements and score; **post-run validation** (`validateTimetableRun`) including **`INCLUDE_ONLY_VIOLATION`**, teacher daily/morning/evening caps, **`SUBJECT_PERIODS_SHORT`**, **`CLASS_FREE_WITH_TEACHER_HEADROOM`** (warning), and optional low-risk autofix on the API route.
...
**Legacy engine env (greedy tuning):** **`LEGACY_ENGINE_RESTARTS`**, **`LEGACY_ENGINE_BACKTRACK_DEPTH`** (default `4`), **`LEGACY_ENGINE_LOCK_REPAIR_ROUNDS`** (default `4`). Optional generate-only object **`legacyEngineOptions`**: `{ restarts?, backtrackDepth?, maxBacktrackRounds?, lockRepairRounds?, localSearchIterations?, localSearchCandidates? }` (stripped before tenant Zod validation, passed through the solver runner).
...
**Create page (web):** The app sends optional **`timetableSolver`** on each `POST /timetable/generate` (pill selector on Create). It overrides **`TIMETABLE_SOLVER` for that request only**; timeouts and sidecar URL still come from env. `report.solver.timetableSolverSource` is **`request`** or **`env`**. **`GET /api/health`** exposes **`timetableSolver.cpSatConfigured`**, **`recommendedUiDefault`** (`hybrid` when the sidecar URL is set or env mode is `hybrid` / `cp_sat`), and **`legacyQualityMaxRecommended`** when the sidecar is absent.

### Legacy quality max (`legacyQualityMax`)

When CP-SAT is unavailable, the API can automatically run the greedy engine with **OPTIMAL-equivalent** tuning for that request only (not persisted to tenant state). Controlled by **`LEGACY_QUALITY_MAX`** (`auto` | `true` | `false`; default **`auto`** = on when **`CP_SAT_SOLVER_URL`** is unset). Logic lives in **`server/legacyQualityProfile.js`**; **`server/timetableSolverRunner.js`** merges a run-only **`classTeacherPreferences.schedulingMode: OPTIMAL`** clone and **`legacyEngineOptions`** (`restarts: 8`, `localSearchIterations: min(80, 24 + 2×divisionCount)`, `lockRepairRounds: 4`, `maxBacktrackRounds: 16`) before **`runTimetableEngine`**. Applies on pure **`legacy`** runs and on **`hybrid` / `cp_sat`** legacy fallback; skipped when **`auto`** and the sidecar URL is set unless **`LEGACY_QUALITY_MAX=true`**. Successful boosted runs set **`report.solver.legacyQualityMax: true`**.
...
- **Manual edits (class view only):** `entries` on the active run are the single source of truth. Division/class UI calls `POST /timetable/valid-edit-targets` (swap vs move-to-free targets), `POST /timetable/valid-add-options` (subjects/teachers for a free cell plus optional **`repairPlans`**), and `POST /timetable/apply-edit` with **`SWAP`**, **`MOVE`**, or **`ADD`**. Placement checks live in `shared/timetablePlacementValidator.js` (engine-parity rules); server persistence in `server/services/timetableManualEditService.js`. Teacher view is read-only and reflects master `entries` after each apply. Client undo for the last **SWAP** uses `manualEdits` snapshots (`src/features/timetable/appActions.js`).
- **Live report recompute:** after each successful apply (and on `GET /timetable/latest`), `shared/recomputeTimetableReport.js` rebuilds `report.unscheduled`, `totalScheduled`, `totalRequired`, `score`, `status`, and lexicographic **`objective`** from current `entries` + `sourceState`, while preserving generate-time diagnostics (`rejections`, `optimization`, `feasibility`, etc.). The client uses `withLiveTimetableReport` so Dashboard/Timetable stay aligned without regenerating.
