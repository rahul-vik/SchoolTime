...
- **Manual edits (class view only):** `entries` on the active run are the single source of truth. Division/class UI calls `POST /timetable/valid-edit-targets` and `POST /timetable/apply-edit`; placement checks live in `shared/timetablePlacementValidator.js` (engine-parity rules). Teacher view is read-only and reflects master `entries` after each apply.
